...
public class GRNFitnessFunctionMulTarDistBalAsymEdgePenalty extends GRNFitnessFunctionMultipleTargetsAllCombinationBalanceAsymmetricZhenyue {
    protected final double edgePenalty;

    public GRNFitnessFunctionMulTarDistBalAsymEdgePenalty(int[][] targets, int maxCycle, double perturbationRate, int[] perturbationSizes, double stride, double edgePenalty) {
        super(targets, maxCycle, perturbationRate, perturbationSizes, stride);
        this.edgePenalty = edgePenalty;
    }

    public GRNFitnessFunctionMulTarDistBalAsymEdgePenalty(int[][] targets, int maxCycle, double perturbationRate, List<Integer> thresholdOfAddingTarget, int[] perturbationSizes, double stride, double edgePenalty) {
...
//        System.out.println("g: " + g);
        double robustness = 1 - Math.pow(Math.E, (-3 * g));
        int grnEdgeNum = GeneralMethods.getEdgeNumber(phenotype);
        double actualEdgePenalty = 0;
        if (grnEdgeNum > 20) {
            actualEdgePenalty = (grnEdgeNum - 20) * this.edgePenalty;
        }
        return robustness - actualEdgePenalty;
    }
}
