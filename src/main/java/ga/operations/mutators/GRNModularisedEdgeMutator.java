 ...
 */
public class GRNModularisedEdgeMutator<T extends Chromosome> implements Mutator<T> {
    private double prob;
    private int moduleIndex;

    public GRNModularisedEdgeMutator(final double prob, final int moduleIndex) {
        filter(prob);
        this.prob = prob;
            ...
            for (Object object : individual.getChromosome().getMaterialsView()) {
                ...
                for (int i=0; i<targetNumber; i++) {
                    ...
                    if (Math.random() <= probabilityToLoseInteraction) { // lose an interaction
                        ...
                        if (i < moduleIndex) {
                            for (int edgeIdx = 0; edgeIdx < moduleIndex; edgeIdx++) {
                                if ((int) material.getGene(edgeIdx * targetNumber + i).getValue() == 0) {
                                    nonInteractions.add(edgeIdx);
                                }
                            }
                        } else {
                            for (int edgeIdx = moduleIndex; edgeIdx < targetNumber; edgeIdx++) {
                                if ((int) material.getGene(edgeIdx * targetNumber + i).getValue() == 0) {
                                    nonInteractions.add(edgeIdx);
                                }
                            }
